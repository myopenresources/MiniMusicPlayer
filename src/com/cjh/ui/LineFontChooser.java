 ...
 *
 */
public class LineFontChooser extends JFrame {
	...
	private static LineFontChooser lineFontChooser = new LineFontChooser();
	
	private   LineFontChooser(){
		init();
	}
	...
	private void init(){
		...
		//读取配置文件
		currentFontName = MusicPlayerConfig.getInstance().getPopString("lineFontName");
		currentFontSize = MusicPlayerConfig.getInstance().getPopString("lineFontSize");
	...
	public static LineFontChooser getInstance(){
		...
		fontNameList.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if(e.getButton()==MouseEvent.BUTTON1 && e.getClickCount()==1){
					...
					MusicSetFrame.getInstance().setLineFontStateLabel(currentFontName,currentFontSize);
					
					//修改歌词显示
					LRCLabel.getInstance().setLineFont(new Font(currentFontName,Font.PLAIN,Integer.valueOf(currentFontSize)));
					//保存配置
					MusicPlayerConfig.getInstance().setPopString("lineFontName", currentFontName);
				}
			}
		});
		...
		fontSizeList.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if(e.getButton()==MouseEvent.BUTTON1 && e.getClickCount()==1){
					...
					LRCLabel.getInstance().setLineFont(new Font(currentFontName,Font.PLAIN,Integer.valueOf(currentFontSize)));
					//保存配置
					MusicPlayerConfig.getInstance().setPopString("lineFontSize", currentFontSize);
				}
			}
		});
